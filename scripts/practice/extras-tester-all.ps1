...
process {
    ...
    # Overall total correct answers
    $totalLanguageQuestions = 0
    $correctLanguageAnswers = 0
    
    foreach ($category in $categoriesList) {
        $categoryObject = $destinationJson[$category]
        ...
        while ($totalQuestions -lt $totalPermutations) {
            # Add 1 to question count
            $totalQuestions++
            $totalLanguageQuestions++
            ...
            $currentResponse = Read-Host ("({0}) ({1}) Translate '{2}'. Accent marks: {3}" -f $languageName, $category, $wordsList[$currentWordNumber], $accentsString)

            # Check response against correct answer, and generate output
            if ($currentResponse.Trim().ToLower() -in ($currentWordAnswer.Split(", "))) {
                $currentOutput, $currentOutputColour, $correctAnswers, $correctLanguageAnswers = ("Correct - {0}" -f $currentWordAnswer), "green", ($correctAnswers + 1), ($correctLanguageAnswers + 1)
            }
            else {
                $currentOutput, $currentOutputColour = ("Wrong - {0}" -f $currentWordAnswer), "red"
            }

            Write-Host ("{0} - {1}/{2} - {3}/{4}" -f $currentOutput, $correctAnswers, $totalQuestions, $correctLanguageAnswers, $totalLanguageQuestions) -ForegroundColor $currentOutputColour
        }
    }
}